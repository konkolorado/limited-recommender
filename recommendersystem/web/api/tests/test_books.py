...
class BookViewPostTestCase(TestCase):
    ...
    def test_api_can_create_book(self):
        response = self.client.post(
            reverse('api-book-create'), self.book, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_api_cannot_create_duplicate_book(self):
        self.client.post(reverse('api-book-create'), self.book, format="json")
        response = self.client.post(reverse('api-book-create'),
...
class BookViewPutTestCase(TestCase):
    ...
    def test_api_cannot_update_fake_book(self):
        self.book["title"] = -1
        response = self.client.put(
            reverse('api-book-detail', kwargs={'pk': self.book["id"] * 50}),
            self.book, format='json'
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
