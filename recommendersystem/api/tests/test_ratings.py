...
class RatingViewPostTestCase(TestCase):
    ...
    def test_api_can_create_rating(self):
        self.response = self.client.post(
            reverse('api-rating-list'), self.new_rating, format="json")
        self.assertEqual(self.response.status_code, status.HTTP_201_CREATED)

    def test_api_cannot_create_duplicate_rating(self):
        self.client.post(reverse('api-rating-list'), self.new_rating,
                         format="json")
        self.response = self.client.post(reverse('api-rating-list'),
...
class RatingViewGetTestCase(TestCase):
    ...
    def test_api_can_get_rating_by_user_id(self):
        self.response = self.client.get(reverse('api-rating-list'),
                                        {'user_id': '0'})
